/*
  ...
  - The values [PENDING] on the enum `inspection_request_status` will be removed. If these variants are still used in the database, this will fail.

*/
-- AlterEnum
BEGIN;
CREATE TYPE "inspection_request_status_new" AS ENUM ('CANCELLED', 'INSPECTING', 'INSPECTED');
...
ALTER TABLE "inspection_request" ALTER COLUMN "status" SET DEFAULT 'INSPECTING';
COMMIT;
